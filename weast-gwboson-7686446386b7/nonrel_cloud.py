...
class NonrelScalar(CloudModel):
  ...
  10808639105689190400), -(8307059966383480541/4722366482869645213696)
    ...
    def omega_real(self, m, alpha, abh, Mcloud):
        ...
        if (not(self._maxalphaInterp(m, alpha, abh))):
            return 1e10
    ...
    def strain_sph_harm(self, m, alpha, abh):
        ...
        omegaGW = 2.0*self.omega_real(m, alpha, spsat, 0)
        h22 = -2.0*np.sqrt(2.0*np.pi*self.power_gw(m, alpha, spsat))/omegaGW
        return np.array([h22])
    def _maxalphaInterp(self, m, alpha, abh):
        """
        Returns True if alpha is below the saturation-alpha for each m at abh
        computed from mu=fc*m*Oh (NOT wr=m*Oh) -> Strictly larger than sat. alpha
        """
        fc = 1.5
        if (m<=self.max_azi_num()):
            return alpha<fc*m*0.5*abh/(1.0+np.sqrt(1.0-abh**2))
        else:
