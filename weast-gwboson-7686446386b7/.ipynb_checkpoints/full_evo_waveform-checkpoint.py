...
class FullEvoWaveform(BosonCloudWaveform):
    def __init__(self, mu, Mbh0, abh0, cloud_model, units="natural"):
        ...
        omega_gw = 2.0*omegaR_vec(self._m, Mbh, abh, Mc/Mbh)/Mbh
    ...
    def freqdot_gw(self, t):
        ...
        Mbh = self._Mbh(t/self._tunit)
        abh = self._abh(t/self._tunit)
        domegaRdMc_vec = np.vectorize(self._cloud_model.domegar_dmc, excluded=[0])
        #Does not currently include effect of Mbh_dot or abh_dot
        fdotgw = (domegaRdMc_vec(self._m, self._mu*Mbh, abh, Mc/Mbh)
                  /Mbh**2/np.pi*Mcdot)
