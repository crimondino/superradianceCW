...
class UltralightBoson(object):
    def __init__(self, spin=1, model="non-relativistic"):
        ...
        if (model=="non-relativistic"):
            ...
            if (spin==0):
                self._cloud_model = RelScalar()
            elif (spin==1):
                self._cloud_model = RelVector()
        else:
            raise ValueError("Model %s not supported" % model)
    def make_waveform(self, Mbh, abh, mu, units="natural", evo_type="matched"):
        if (evo_type=="matched"): 
